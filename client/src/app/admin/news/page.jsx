...
export default function AdminNews() {
  ...
  const handleGalleryImages = (e) => {
    ...
    setImagePreviews((prev) => ({
      ...prev,
      galleryImages: [...prev.galleryImages, ...files.map((file) => URL.createObjectURL(file))],
    }));
  };
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {loading ? (
              ...
              <p className="text-gray-500 text-center col-span-full">No news found.</p>
            ) : (
              newsList.map((n) => (
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 overflow-auto">
          <div className="bg-white rounded-xl shadow-xl max-w-5xl w-full my-10 p-8 relative">
            ...
            <h2 className="text-2xl font-semibold mb-6 text-black">Create News</h2>
            {message && (
            ...
            <form className="space-y-6" onSubmit={handleSubmit} encType="multipart/form-data">
              ...
              <button type="submit" className="bg-gray-900 text-white px-6 py-3 rounded hover:bg-gray-800 transition">{loading ? "Saving..." : "Create News"}</button>
            </form>
          </div>
        </div>
      )}
